# Змейка — мультиплеер по API

Сервер и веб-интерфейс для многопользовательской змейки. Игра идёт по API: твой код получает состояние мира (стенки, яблоки, змейки) и отправляет один ход за тик — вверх/вниз/влево/вправо. 4 тика в секунду, раунд 3 минуты, 5 уровней.
...
Из папки `pack_for_students`. URL по умолчанию в коде — `http://127.0.0.1:8002` (локальный сервер). Логин и пароль бери из таблицы в админке после старта игры:
...
Для локального сервера URL можно не указывать (по умолчанию уже `http://127.0.0.1:8002`):  
`python bot.py player_1 ПАРОЛЬ_ИЗ_АДМИНКИ`

Четвёртый аргумент — имя игрока (по желанию): `python bot.py http://127.0.0.1:8002 player_1 ПАРОЛЬ МойБот`
...
- Сервер **тикает 4 раза в секунду**. За один тик каждый игрок может отправить **один ход** (направление).
...
- **main.run()** в бесконечном цикле:
  ...
  6. Спит ~0.25 сек до следующего тика.
