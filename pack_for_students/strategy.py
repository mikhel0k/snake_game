...
from pack_for_students.world import WorldView, next_head
...
# =============================================================================
# По умолчанию — «глупый» бот: цели нет, только объезд препятствий (стены, тела змеек).
# Запасной ход: по возможности прямо, иначе случайное безопасное направление.
# Чтобы бот стремился к яблокам, подставь свои цели и pathfinder (см. комментарии выше).

# Цель: нет — бот только уворачивается. Подставь [nearest_apple] или [nearest_reachable_apple, ...] для погони за яблоками.
DEFAULT_GOAL_SELECTORS = [no_goal]
# Путь к цели (при наличии цели): по прямой (Манхэттен). При no_goal не вызывается.
DEFAULT_PATHFINDER = step_toward_manhattan
# Запасной ход — когда цели нет или путь не найден: прямо, иначе случайный безопасный ход.
